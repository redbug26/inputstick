...
char process_data(uint8_t *data, size_t data_len, le_advertising_info *info, char *returnAddr)
{
    char stop = 0;
    ...
    if (data[0] == EIR_NAME_SHORT || data[0] == EIR_NAME_COMPLETE) {
        ...
        if (!strcmp(name, "InputStick")) {
            strcpy(returnAddr, addr);
            stop = 1;
        }
...
void scan(struct hci_state *current_hci_state, char *returnData)
{
        ...
        if (!done && !error) {
            ...
            while (!data_error && current_index < info->length) {
                ...
                if (data_len + 1 > info->length) {
                    ...
                    char stop = process_data(info->data + current_index + 1, data_len, info, returnData);
